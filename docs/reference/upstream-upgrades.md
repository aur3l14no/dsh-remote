...
| 原生 Session 日志格式 | 上游 Session 存储在宿主。本项目只验证固定版本生成的当前格式；研究阶段不维护旧日志迁移 fixture，格式变化时显式重置实验状态 | 当前格式冷恢复、fork、child 与 binding 保持测试 |
...
研究阶段固定一个 revision，按实验需要升级。新增补丁应直接服务执行身份、生命周期或 provider 接口；展示便利优先使用现有插件槽位。New Session 使用原生控件，Reload worlds 位于工作区插件，不再维护侧栏 primary actions 补丁。
