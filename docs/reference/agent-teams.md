...
标准与 SSH preset 使用 rc.2 的 experimental Agent Teams。Agent runtime、成员目录、消息与任务板都留在 DSH 宿主；项目文件和进程由每个成员的 World binding 或单次工具环境路由。
...
原生 `subagent` 用于一次性委派；`send_message`、`list_agents`、`interrupt_agent` 面向 Team roster 中的成员。

## 浏览器

会话顶部的 Agent Team 打开成员列表和共享任务板，可以编辑任务并打开 teammate 会话。面板在打开、手动刷新和修改时读取快照；它不是实时 mailbox 时间线。普通子 Agent 导航仍由原生目录管理。

这是 experimental 功能，固定到与宿主一致的 rc.2。协调限一个 DSH 进程。停机和中断不会自动释放任务 owner。
