...
功能与执行位置见[系统全景](../system-map-1.md)。本页保留路径、存储与身份校验的精确规则。
...
文件预览不提供 OS 文件 watcher；变更提示来自 Agent FS observation，外部编辑需手动刷新。helper 缺少范围读取 capability 时明确拒绝，不用整文件下载模拟范围读取。
...
SSH Session 上传的图片、原样文件以及 `read_image` 生成的图片对象，以远端为持久源。存储位于远端账户 `$HOME/.local/share/dsh-remote/attachments/v1/<Workspace 指纹>/`，独立于项目目录、helper 安装和临时 runtime。指纹由完整不可变 Workspace 执行身份生成；不同 World 不因内容摘要相同而共享引用。Session 继续使用原生日志格式，附件 ID 携带内容摘要和 Workspace 指纹（编码仍使用 `@world-` 标签）。

上传、工具、模型转换、历史预览和 Session 日志导出通过 `attachments.forSession(sessionId)` 选择后端。写入先原子发布并完成 `fs.sync`，再返回可写入日志的引用；缺少该 capability 的 helper 明确拒绝附件写入。模型提示里的文件/图片读取路径属于远端账户，宿主缓存路径不会作为工具路径给模型。
...
SSH Session 拒绝不带 Workspace 指纹的裸 `sha256:<摘要>` 引用，不读取旧宿主 attachment store，也不自动迁移。需要的附件在当前绑定的会话中重新上传。本机 Session 仍使用原生裸引用；只有经过 local binding 校验后才委托本机后端。

自定义 profile 如直接配置被替换的附件、LLM、file-upload、subagent 或导出行，应改用 overlay 中对应的 `remote-*` 行 ID；Models 设置页与 `settings.yaml` 的 provider 配置键不变。
