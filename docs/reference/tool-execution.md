...
路由由工具的执行环境声明启用，参数校验由各工具 schema 负责。原生 `subagent` 和 `spawn_teammate` 的同名参数接收 Workspace ID 字符串，为新成员选择持续使用的环境；用法见[跨 World 委派](cross-world-delegation.md)。
...
工具参数原样写入原生 `tool/call`。原生 `approval/request` 通过 callId 关联工具调用，实际 World 事实来自当前调用环境。
...
显式环境下的 write/edit 结果与 present 交付保存带来源的文件引用。它包含 World ID、canonical 路径、调用目录和环境指纹，不包含 SSH 凭据。工具定义通过统一环境接口提供文件环境 metadata；工具详情的文件链接、write/edit 的输出链接与 present 交付都据此打开实际目标。metadata 使用原生结果载体。工具尚未完成时不开放详情中的文件链接；显示名称省略机器使用的查询字段。
