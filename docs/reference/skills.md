...
本项目复制选定的 skill 文件夹，不运行安装脚本。独立部署配置只列出需要部署 skills 的 SSH 条目：
...
源根可以是链接，内部只接受自包含的普通文件和目录。单个 skill 限 4096 个文件、32 MiB。请勿把含认证或私人配置的目录作为 skill 源。
...
本地脚本路径、宿主应用和认证不会随部署变得远端可用；需要适配远端命令或使用独立本地连接器，不能自动改写 shell 或回退宿主。
