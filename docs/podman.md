...
`scripts/accept-podman.mjs` requires explicit environment inputs:
...
```sh
node scripts/accept-podman.mjs
```

After `pack-plugin.mjs` and `check-plugin.mjs`, run `node scripts/accept-podman.mjs --package` with the same inputs to exercise the unpacked plugin's Loader entries and Session routing in a second disposable container. It writes `target/podman-package-acceptance.json` with the tarball integrity and digest.
