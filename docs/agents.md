...
Remaining work includes the PTY and Session-job adapters, subagent tool restrictions/continuations, complete application consumer review and independently installed package verification. The current helper protocol, reconnection semantics, filesystem size/listing limits and cleanup boundaries are unchanged. Release downloading, signed catalogs and installation garbage collection remain separate work.
