...
| workspace Shell、PTY、进程、信号与输出 | provider 传 argv/cwd/env 到远端 helper | 前台/后台 Bash、jobs 查询与取消已有浏览器验收；PTY UI 不在当前 preset 中 |
...
当前实现只通过独立部署命令复制配置选定的 skill 文件夹；不会因读取这条指令而临时复制脚本、改写 shell 字符串或在失败后重试本地。远端同名路径存在时也不能把它当成本地脚本的等价物。单独设置 execution-world 提示词不能解决文件、依赖和权限问题。
