  ...
  "worlds": [
    {
      ...
      "name": "This computer",
      "target": { "kind": "local" },
      "workspaces": [{ "name": "Local project", "path": "/Users/me/projects/example" }]
    },
    {
      "id": "development",
      "name": "Dev server",
...
侧边栏底部的刷新图标（**Reload worlds**）也支持手动检查：即使 `worlds.json` 没变，也能同步本地 skill 来源的内容更新。界面每两秒检查配置状态；没有 SSH skill 来源文件 watcher。本机目录预览只校验路径，使用原生 skill 发现，不进入远端部署流程。
