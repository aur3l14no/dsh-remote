...
| Web activation | Direct creation calls local `mkdir` before World admission; cold activation lacks preparation. Its owned lookup rejects a second resolver. | Reproduced in the [Project experiment](../experiments/project-worlds/README.md). Accepted limitation; no upstream-change dependency. |
