...
/** Uses an already installed helper/socket. Provisioning is a separate next-stage responsibility. */
...
export function connectSuppliedRuntime(target: SuppliedRuntime): Promise<Client> {
  return Client.open({ world: target.world, connect: sshTransport(target), required: ['fs.bytes', 'fs.atomic-publish', 'process.pipe', 'output.ack', 'output.collect'] });
}
