...
import { sshControl } from '../packages/ssh/src/control.ts';
import { bootstrapSshWorld } from '../packages/ssh/src/index.ts';
...
try {
  ...
  if (packaged) {
    ...
    try {
      await run('client', ['--test', 'tests/client/ssh.test.ts'], { ...env, DSH_TEST_REMOTE_HELPER: ready.installation.helper, DSH_TEST_REMOTE_RG: ready.ripgrep });
    } finally { await ready.close(); }
    await run('bootstrap', ['--test', 'tests/bootstrap/install.test.ts'], env);
    for (const suite of ['session-routing', 'terminal']) {
      await run(`build-${suite}`, ['scripts/build-composition.mjs', source, suite], env);
