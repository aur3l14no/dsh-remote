...
if (!process.argv[2]) throw new Error('Usage: node scripts/check-plugin.mjs DSH_SOURCE_CHECKOUT');
const buildRecord = JSON.parse(await readFile('target/packages/plugin-build.json', 'utf8'));
if (execFileSync('git', ['-C', root, 'rev-parse', 'HEAD'], { encoding: 'utf8' }).trim() !== buildRecord.dshSourceRevision
    || execFileSync('git', ['-C', root, 'status', '--porcelain', '--untracked-files=no'], { encoding: 'utf8' }).trim()) throw new Error('DSH baseline changed');
...
const remotes = new Map([
  [resolve('packages/dsh-ssh/src/worlds.ts'), '@dsh-remote/ssh-world'],
  [resolve('packages/dsh-ssh/src/routing.ts'), '@dsh-remote/ssh-world/routing'],
  [resolve('packages/dsh-ssh/src/bindings.ts'), '@dsh-remote/ssh-world/bindings'],
  [resolve('packages/client/src/index.ts'), '@dsh-remote/ssh-world/client'],
]);
const result = await build({ entryPoints: ['tests/integration/session-routing.ts'], outfile: join(output, 'accept.mjs'),
  ...
  plugins: [{ name: 'installed-plugin', setup(builder) {
    builder.onResolve({ filter: /(?:dsh-ssh|client)\/src\// }, ({ path, resolveDir }) => {
...
if (Object.keys(result.metafile.inputs).some(file => file.includes('packages/dsh-ssh/'))) throw new Error('Host fixture bundled plugin source');
