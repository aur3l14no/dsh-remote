...
const program = ts.createProgram([...['world', 'context', 'fs', 'subprocess', 'terminal', 'bindings', 'worlds', 'routing'].map(name => join(local, 'src', name + '.ts')), ...['composition', 'remote-runtime', 'agents', 'preset-harness', 'terminal-consumers', 'terminal', 'session-routing', 'project-worlds'].map(name => resolve('tests/integration', name + '.ts'))], {
...
const diagnostics = ts.getPreEmitDiagnostics(program).filter(d => !d.file || ['packages', 'experiments', 'tests/integration'].some(dir => d.file.fileName.startsWith(resolve(dir) + '/')));
