...
The DSH composition fixture accepts `DSH_TEST_BOOTSTRAP_MANIFEST` and `DSH_TEST_ARTIFACT_CACHE` instead of a supplied remote helper/ripgrep pair. Its harness and consumers stay local while helper installation and workspace execution happen remotely. See [measured bootstrap results](evidence/bootstrap-acceptance-results.json).
