...
The application places the existing terminal, Bash backend, jobs and tool plugins beside the remote providers in a standing DSH preset. The checked-in example is `integrations/dsh/tests/integration/terminal-consumers.ts`, called by the acceptance harness; it is not a runtime/tool policy shipped by the World plugin.
...
node integrations/dsh/scripts/build-composition.mjs "$DSH_SOURCE" terminal
