...
The [Project experiment](../../../../integrations/dsh/experiments/portable_workspace/README.md) now verifies that preparing a bound native Agent lets Web's activation controllers adopt its explicit ID without local mkdir. Explicit Project reopen also restores native JSONL Sessions. Direct cold Web activation still skips World preparation, and direct new-Session creation still makes a local directory. This is controller-level evidence, not a browser demo.
