...
import { Menu, Tooltip, IconFolderClose16, IconArchiveOutline20, type MenuEntry } from '@deepseek-ai/dsh-client-ui-primitives';
...
function installWorkspaceUi(ctx: Context) {
  ...
  function WorkspacePicker({ open, anchorRef, selectedId, onPick, onClose }: PropsRuntime<'conversation.hero.workspace'>) {
    ...
    const items: MenuEntry[] = worlds.filter(world => world.workspaces.length).flatMap((world, index) => [
  ...
  ctx.slots.inject('conversation.hero.workspace', () => ctx.slots.register({ name: 'conversation.hero.workspace' }, WorkspacePicker));
}
