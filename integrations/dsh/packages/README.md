...
`bundle/remote/src/index.ts` mounts skill synchronization and World ownership, publishes the workspace feed, then mounts the registry and consumers. The feed must be active before the registry releases the controller. The [overlay and preset](../profiles/README.md) mount native services and isolated tool routers separately. Browser code stays with workspace at `workspace/portable-workspace/src/client/index.tsx`.
