...
export function apply(ctx: Context, config: Config = {}): void {
  ...
  ctx.systemPrompt.context({ name: 'execution-world', order: -1000,
    text: context => `Execution World (workspace operations execute here):\n${JSON.stringify(executionWorldContext(ctx, context.agent!))}\nThe world field is your World ID; workspace is your Workspace ID. Workspace tools default to this binding. Tools advertising execution_environment accept {world, cwd} for this call only; use configured World IDs and an existing absolute cwd. The Session workspace and its instructions remain unchanged. To discover other configured Worlds, call list_worlds. From a local Session, pass a returned World id to prepare_workspace.world_id, then pass its executionEnvironment to subagent.execution_environment. Never substitute a World ID or display name for a Workspace ID.` });
}
