...
import { bootstrapSshWorld, connectSuppliedRuntime, sshArguments } from '../../../../runtime/ssh/src/index.ts';
...
export async function remoteRuntime(world: string) {
  ...
  if (manifest) {
    ...
    const ready = await bootstrapSshWorld({ ...target, world, cacheDir: process.env.DSH_TEST_ARTIFACT_CACHE,
      manifest: JSON.parse(await readFile(manifest, 'utf8')), installRoot: `${root}/artifacts` });
    return { client: ready.client, dir: root, ripgrep: ready.ripgrep, async close() { await ready.close(); await command(['chmod', '-R', 'u+w', root]); await command(['rm', '-r', root]); } };
  }
  assert.ok(helper);
  try {
    await command([helper, 'start', '--runtime-dir', `${root}/runtime`, '--grace-ms', '15000', '--lease-ms', '5000']);
    const client = await connectSuppliedRuntime({ ...target, world, helper, socket: `${root}/runtime/socket` });
    return { client, dir: root, async close() {
