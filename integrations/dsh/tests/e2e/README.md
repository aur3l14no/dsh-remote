...
| previews and cold restore | Host/two-World same-path isolation, range/full/related reads, Sidebar/images/present, allowed outside-root files, symlink and outside-root directory rejection, and change filtering; current-format cold Session restore with saved bindings and target file identity preserved |
...
| `worlds-reload.mjs` | Read-only preview/cancel, frozen sources, stale config/remote-state rejection, stored-version drift, managed unlink, destination conflicts, partial failure and retry across two SSH Worlds |
| `connect-install.mjs` | Clean startup, worlds.json hot reload dialog, cancellation, stale approval, retired World Session preservation, deferred automatic runtime download and retry, offline restart, hover pin/archive controls, persistent pins, truncated titles/detail tooltip and stale picker response cancellation |
| `extension-install.mjs` | Official CLI installs the extension tarball and initializes through init-release, then Playwright exercises first-run welcome, Session creation, configured automatic sync/independent deployment, failed-source retry and unchanged helper PIDs; no scaffold |
