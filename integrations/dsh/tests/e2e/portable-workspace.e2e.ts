...
it('keeps portable workspaces isolated across the Web lifecycle and failures', async () => {
  ...
  await expect.poll(() => cardFor(scaffold!.ctx.agents.list()[0]!.session.header.id).locator('svg').getAttribute('stroke')).toBe('#a855f7');
  expect(await cardFor(scaffold.ctx.agents.list()[1]!.session.header.id).locator('svg').getAttribute('stroke')).not.toBe('#a855f7');
