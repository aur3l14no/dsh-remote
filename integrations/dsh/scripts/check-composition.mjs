...
const program = ts.createProgram([...['workspace', 'context', 'fs', 'subprocess', 'terminal', 'jobs', 'bindings', 'worlds', 'routing'].map(name => join(local, ['bindings', 'worlds', 'routing', 'jobs'].includes(name) ? '../execution-world/src' : 'src', name + '.ts')), ...['composition', 'remote-runtime', 'agents', 'preset-harness', 'terminal-consumers', 'terminal', 'session-routing', 'local-world', 'portable_workspace', 'shared-runtime'].map(name => resolve('integrations/dsh/tests/integration', name + '.ts'))], {
