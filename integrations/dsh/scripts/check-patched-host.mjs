...
import { access, copyFile, symlink } from 'node:fs/promises';
...
const childEntry = resolve('integrations/dsh/tests/patched-host/child-environment.ts');
...
await build({ entryPoints: { admission: entry, 'child-environment': childEntry }, outdir: join(output, 'lib'), outExtension: { '.js': '.mjs' }, bundle: true, platform: 'node', format: 'esm',
  ...
  plugins: [{ name: 'patched-host-source', setup(builder) {
    builder.onResolve({ filter: /^(@deepseek-ai\/|@dsh-test\/)/ }, async ({ path }) => {
...
if (!existsSync(join(output, 'lib/node_modules'))) await symlink(resolve('.build/dsh/official-install/node_modules'), join(output, 'lib/node_modules'));
