...
The overlay replaces local workspace discovery/navigation and related controllers with the World registry and admission adapter. The local provider retains native directory selection and sandbox behavior. The SSH preset excludes same-cwd Session references, host file references, desktop path opening, local sandbox runners and unadapted consumers. Both presets route each tool call through its resolved World/cwd; the local shell facade retains the native sandbox context. The remote preset groups FS/subprocess/shell/workdir services with native file/search, Bash/jobs, Web/skill, instructions, subagent and terminal consumers. Activation and module identity rules are in [system map](../../../../docs/system-map.md).
